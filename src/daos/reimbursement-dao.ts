...
export async function getAllReimbursements():Promise<Reimbursement[]>{
    ...
    try {
        ...
        let results:QueryResult = await client.query(`select r.*, rs.status, rs."status_id", rt."type", rt."type_id" from project-0-rkennedy98.reimbursement r
                                                        join project-0-rkennedy98.reimbursement_status rs on r.status = rs.status_id
                                                        join project-0-rkennedy98.reimbursement_type rt on r."type" = rt.type_id
...
export async function getReimbursementByUserId(userId:number):Promise<Reimbursement[]> {
    ...
    try {
        ...
        let results = await client.query(`select r."reimbursement_id", 
                                                ...
                                                rt."type_id", rt."type"
                                            from project-0-rkennedy98.reimbursements r 
                                            left join project-0-rkennedy98.reimbursement_statuses rs
                                                on r."status" = rs."status_id" 
                                            left join project-0-rkennedy98.reimbursement_types rt
                                                on r."type" = rt."type_id"
                                            left join project-0-rkennedy98.users u 
...
export async function saveOneReimbursement(newReim:Reimbursement):Promise<Reimbursement> {
    ...
    try {
        ...
        let typeId = await client.query(`select t."type_id" from project-0-rkennedy98.reimbursement_types t 
                                            where t."type" = $1;`,
                                        [newReim.type])
        ...
        let statusId = await client.query(`select rs."status_id" from project-0-rkennedy98.reimbursement_statuses rs 
                                            where rs."status" = $1;`, [newReim.status])
        if(statusId.rowCount === 0) {
            throw new Error('Status Not Found')
        }
        statusId = statusId.rows[0].status_id
        let results = await client.query(`insert into project-0-rkennedy98.reimbursements ("author", "amount", 
...
export async function updateReimbursement(updatedReimbursement:Reimbursement):Promise<Reimbursement> {
    ...
    try {
        ...
        if(updatedReimbursement.status) {
            ...
            statusId = statusId.rows[0].status_id
            await client.query(`update project-0-rkennedy98.reimbursements set "status" = $1 
                                where "reimbursement_id" = $2;`, 
                                [statusId, updatedReimbursement.reimbursementId])
        }
        if(updatedReimbursement.type) {
            let typeId = await client.query(`select rt."type_id" from project-0-rkennedy98.reimbursement_types rt 
                                            where rt."type" = $1;`, [updatedReimbursement.type])
            if(typeId.rowCount === 0) {
                throw new Error('Type Not Found')
            }
            typeId = typeId.rows[0].type_id
            await client.query(`update project-0-rkennedy98.reimbursements set "type" = $1 
                                where "reimbursement_id" = $2;`, 
